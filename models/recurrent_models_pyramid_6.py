...
class LSTMGenerator(nn.Module):
    ...
    def __init__(self, in_dim, out_dim, device=None):
        ...
        #self.lstm0 = nn.LSTM(in_dim, hidden_size=64, num_layers=1, batch_first=True)
        ...
        #self.linear = nn.Sequential(nn.Linear(in_features=64, out_features=out_dim), nn.Tanh())
    ...
    def forward(self, input):
        ...
        h_0 = torch.zeros(1, batch_size, 32, device = torch.device('cuda'))
        c_0 = torch.zeros(1, batch_size, 32, device = torch.device('cuda'))
        ...
        #outputs = self.linear(recurrent_features.contiguous().view(batch_size*seq_len, 64))
...
class LSTMDiscriminator(nn.Module):
    ...
    def __init__(self, in_dim, device=None):
        ...
        self.linear = nn.Sequential(nn.Linear(in_features=64, out_features=1), nn.Sigmoid())


    def forward(self, input): #batch_size 100인데 32로 바꿈...
        #batch_size, seq_len= input.size(0), input.size(1)
        batch_size, seq_len = input.size(0), input.size(1)
        
        #h_0 = torch.zeros(1, batch_size, 100).to(self.device)
        #c_0 = torch.zeros(1, batch_size, 100).to(self.device)
        h_0 = torch.zeros(1, batch_size, 64, device = torch.device('cuda'))
        c_0 = torch.zeros(1, batch_size, 64, device = torch.device('cuda'))
